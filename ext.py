...
if os.name == 'Linux' or os.name == "posix":
    ...
    for char in '\033[1;31m\t \t{+} Usage: \033[1;m' '\033[1;34mpython\033[1;m ' + sys.argv[
        0] + '\033[1;34m 127.0.0.1\033[1;m':
    ...
    try:
        ...
        s = sys.argv[1]
        while page <= 30:
            bing = "http://www.bing.com/search?q=ip%3a" + s + "+/user/login+&first=" + str(page)
            openbing = urllib2.urlopen(bing)
            readbing = openbing.read()
            findwebs = re.findall('<h2><a href="(.*?)"', readbing)
            for i in range(len(findwebs)):
